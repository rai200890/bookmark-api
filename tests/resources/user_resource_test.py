...
def test_get_exists(api_test_client, user, auth_headers):
    response = api_test_client.get('/users/{}'.format(user.id), headers=auth_headers)
...
def test_get_doesnt_exist(api_test_client, auth_headers):
    response = api_test_client.get('/users/{}'.format(0), headers=auth_headers)
    assert response.status_code == 404


def test_delete_exists(api_test_client, user, auth_headers):
    response = api_test_client.delete('/users/{}'.format(user.id), headers=auth_headers)
    assert response.status_code == 204


def test_delete_doesnt_exists(api_test_client, auth_headers):
    response = api_test_client.delete('/users/{}'.format(0), headers=auth_headers)
...
def test_post_invalid(api_test_client, create_invalid_params, auth_headers):
    response = api_test_client.post('/users',
...
def test_put_valid(api_test_client, user, edit_valid_params, auth_headers):
...
def test_put_invalid(api_test_client, user, other_user, edit_invalid_params, auth_headers):
